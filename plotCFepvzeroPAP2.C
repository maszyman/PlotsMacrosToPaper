...
void plotCFepvzero(const char* sys="PP", const char* kT="kT1", const char* proj="Out", int mult=2, TString dir = "../train_results_central_dcaptdep", int period=11) {
...
//        densum->Draw("p");
    ...
    if (strcmp(sys,"PAP2") != 0) {
      ...
      numpp[0][2][Nruns] = (TH1D*)ifile_[Nruns]->Get(Form("Num%sPckstarPAPtpcM%dPsi%d%s",proj,mult,6,kT));
      denpp[0][2][Nruns] = (TH1D*)ifile_[Nruns]->Get(Form("Den%sPckstarPAPtpcM%dPsi%d%s",proj,mult,6,kT));

      numpp[0][0][Nruns] = (TH1D*)ifile_[Nruns]->Get(Form("Num%sNckstarPAPtpcM%dPsi%d%s",proj,mult,6,kT));
      denpp[0][0][Nruns] = (TH1D*)ifile_[Nruns]->Get(Form("Den%sNckstarPAPtpcM%dPsi%d%s",proj,mult,6,kT));
  ...
  else if (strcmp("PAP",sys)==0)
    Tl.DrawLatex(0.22,1.093,"p#bar{p}, ");
  else if (strcmp("APAP",sys)==0)
