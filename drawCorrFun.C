...
void drawCorrFun (const char* infilename, const char* system, const char* field, Int_t rWrite, Int_t rPerformance, Int_t kT)
{
  ...
  for (int j = 0; j < multBins; ++j) {
    ...
    if (!kT) {
      if (strcmp(system,"PP") == 0)
        sprintf(numName,"NumcqinvPPtpcM%dPsi6",j);
      else if (strcmp(system,"APAP") == 0)
        sprintf(numName,"NumcqinvAPAPtpcM%dPsi6",j);
      else if (strcmp(system,"PAP") == 0)	{
        // sprintf(numName,"NumcqinvPAPtpcM%d",j);

        sprintf(numName,"Num%s%sckstarPAPtpcM%dPsi6",osl,sig,j);
        sprintf(numNameN,"Num%s%sckstarPAPtpcM%dPsi6",osl,sig,j);
      }
      if (strcmp(system,"PP") == 0)
        sprintf(denName,"DencqinvPPtpcM%dPsi6",j);
      else if (strcmp(system,"APAP") == 0)
        sprintf(denName,"DencqinvAPAPtpcM%dPsi6",j);
      else if (strcmp(system,"PAP") == 0)	{
        // sprintf(denName,"DencqinvPAPtpcM%d",j);

        sprintf(denName,"Den%s%sckstarPAPtpcM%dPsi6",osl,sig,j);
        sprintf(denNameN,"Den%s%sckstarPAPtpcM%dPsi6",osl,sig,j);
      }
    }
    else {
      if (strcmp(system,"PP") == 0)
        sprintf(numName,"NumcqinvPPtpcM%dPsi6kT%d",j,kT-1);
      else if (strcmp(system,"APAP") == 0)
        sprintf(numName,"NumcqinvAPAPtpcM%dPsi6kT%d",j,kT-1);
      else if (strcmp(system,"PAP") == 0)	{
        // sprintf(numName,"NumcqinvPAPtpcM%dPsi6kT%d",j,kT);

        sprintf(numName,"Num%s%sckstarPAPtpcM%dPsi6kT%d",osl,sig,j,kT-1);
        sprintf(numNameN,"Num%s%sckstarPAPtpcM%dPsi6kT%d",osl,sig,j,kT-1);
      }
      if (strcmp(system,"PP") == 0)
        sprintf(denName,"DencqinvPPtpcM%dPsi6kT%d",j,kT-1);
      else if (strcmp(system,"APAP") == 0)
        sprintf(denName,"DencqinvAPAPtpcM%dPsi6kT%d",j,kT-1);
      else if (strcmp(system,"PAP") == 0)	{
        // sprintf(denName,"DencqinvPAPtpcM%dPsi6kT%d",j,kT);

        sprintf(denName,"Den%s%sckstarPAPtpcM%dPsi6kT%d",osl,sig,j,kT-1);
        sprintf(denNameN,"Den%s%sckstarPAPtpcM%dPsi6kT%d",osl,sig,j,kT-1);
      }
    }
