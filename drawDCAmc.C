...
void drawDCAmc(const char* infilename, const char* system, const char* status, Int_t rWrite, Int_t rPerformance, int isMC, Int_t bin, Int_t ptrange)
{
  ...
  if (!isMC) {
    ...
    TH2D* matp =(TH2D*)f->Get(Form("DCARPtcut%s1%stpcM%dmate","Pass", system,0));
    ...
    TF1* ff = new TF1("asd","0.018 + 0.036*TMath::Power(x, -1.01)",0.1,2);
    ff->Draw();
...
// //asdasdadsadsadsdasd
    ...
    myPadSetUp(padsum,0.15,0.1,0.04,0.15);
    padsum->Draw();
    padsum->SetLogy();
    ...
    if (strcmp(system,"PP") ==0)
      asd0->SetTitle("protons, 0.5 < #it{p}_{T} < 3 GeV/#it{c}");
    else
      asd0->SetTitle("antiprotons, 0.3 < #it{p}_{T} < 3 GeV/#it{c}");

    if (strcmp(system,"PP") ==0)
      primpp->SetTitle("protons, 0.5 < #it{p}_{T} < 3 GeV/#it{c}");
    else
      primpp->SetTitle("antiprotons, 0.3 < #it{p}_{T} < 3 GeV/#it{c}");

    asd0->GetXaxis()->SetTitle("DCA_{xy} (cm)");
    asd0->GetYaxis()->SetTitle("Number of Entries");
    ...
    primpp->GetXaxis()->SetTitle("DCA_{xy} (cm)");
    primpp->GetYaxis()->SetTitle("Number of Entries");
    // primpp->GetYaxis()->SetTitle("Number of Entries (normalized)");
    //primpp->SetMaximum(5000);
    primpp->SetMinimum(1);
    primpp->GetXaxis()->SetNdivisions(8);
    primpp->GetYaxis()->SetNdivisions(8);
    //primpp->GetYaxis()->SetTitleOffset(1.4);
    primpp->GetXaxis()->SetTitleSize(0.068);
    primpp->GetYaxis()->SetTitleSize(0.068);
    primpp->GetXaxis()->SetLabelSize(0.058);
    primpp->GetYaxis()->SetLabelSize(0.058);
    primpp->SetFillColor(kBlack);
    ...
    TLatex *sys = new TLatex(0.16,0.8,"#splitline{HIJING LHC11a10a_bis}{AOD120 Pb-Pb #sqrt{s_{NN}} = 2.76 TeV}");
    sys->SetNDC();
    sys->SetTextFont(42);
    sys->SetTextSize(0.04);
