...
void drawPID(const char* infilename, const char* system, const char* status, Int_t rWrite, Int_t rPerformance, Int_t bin)
{
  ...
  if (!bin) {
    ...
    TH2D* TPCdEdx =(TH2D*)f->Get(Form("TPCdEdxcut%s1%stpcM%iPsi%d",status, system,bin-1,psibins));
	...
	for (int i = minMultBin; i < maxMultBin; i++) {
		...
		TH2D* TPCdEdxN =(TH2D*)f->Get(Form("TPCdEdxcut%s1%stpcM%iPsi%d",status,system,i,psibins));
    cout << Form("TPCdEdxcut%s1%stpcM%iPsi%d",status,system,i,psibins) << endl;
...
// 	py->Draw();
	...
	// TPC Nsigma
  if (!bin)
    TH2D* TPCNsigma =(TH2D*)f->Get(Form("TPCNSigmacut%s1%stpcM%iPsi%d",status,system,0,psibins));
  else
    TH2D* TPCNsigma =(TH2D*)f->Get(Form("TPCNSigmacut%s1%stpcM%iPsi%d",status,system,bin-1,psibins));
	...
	for(int i = minMultBin; i<maxMultBin; i++) {
		//all
		TH2D* TPCNsigmaN =(TH2D*)f->Get(Form("TPCNSigmacut%s1%stpcM%iPsi%d",status,system,i,psibins));
	...
	// TOF Nsigma
  if (!bin)
    TH2D* TOFNsigma =(TH2D*)f->Get(Form("TOFNSigmacut%s1%stpcM%iPsi%d",status,system,0,psibins));
  else
    TH2D* TOFNsigma =(TH2D*)f->Get(Form("TOFNSigmacut%s1%stpcM%iPsi%d",status,system,bin-1,psibins));
	...
	for(int i = minMultBin; i<maxMultBin; i++) {
		//all
		TH2D* TOFNsigmaN =(TH2D*)f->Get(Form("TOFNSigmacut%s1%stpcM%iPsi%d",status,system,i,psibins));
	...
	// TOF time
  if (!bin)
    TH2D* TOFTime =(TH2D*)f->Get(Form("TOFTimecut%s1%stpcM%iPsi%d",status,system,0,psibins));
  else
    TH2D* TOFTime =(TH2D*)f->Get(Form("TOFTimecut%s1%stpcM%iPsi%d",status,system,bin-1,psibins));
	...
	for(int i = minMultBin; i<maxMultBin; i++) {
		//all
		TH2D* TOFTimeN =(TH2D*)f->Get(Form("TOFTimecut%s1%stpcM%iPsi%d",status,system,i,psibins));
	...
	// TPC & TOF Nsigma
  if (!bin)
    TH2D* TPCTOFNsigma =(TH2D*)f->Get(Form("TPCTOFNSigmacut%s1%stpcM%iPsi%d",status,system,0,psibins));
  else
    TH2D* TPCTOFNsigma =(TH2D*)f->Get(Form("TPCTOFNSigmacut%s1%stpcM%iPsi%d",status,system,bin-1,psibins));
	...
	for(int i = minMultBin; i<maxMultBin; i++) {
		//all
		TH2D* TPCTOFNsigmaN =(TH2D*)f->Get(Form("TPCTOFNSigmacut%s1%stpcM%iPsi%d",status,system,i,psibins));
		TOFNsigma->Add(TOFNsigmaN);
		//delete hEvMult;
	}
