void plotcomparisonmult(const char* infilename1, const char* infilename2, const char* infilename3, const char* system, const char* kT, const char* cmp) {


  TFile* infile1 = new TFile(infilename1,"read");
  TFile* infile2 = new TFile(infilename2,"read");
  TFile* infile3 = new TFile(infilename3,"read");
  ...
  if ( strcmp(system,"PAP") == 0 ) {
    ...
    cfv1 = (TH1D*)infile1->Get(Form("NumOutPckstarPAPtpcM0Psi3%s",kT));
    cfv2 = (TH1D*)infile2->Get(Form("NumOutPckstarPAPtpcM2Psi3%s",kT));
    cfv3 = (TH1D*)infile3->Get(Form("NumOutPckstarPAPtpcM4Psi3%s",kT));


    cfv1->SetMaximum(1.12);
    cfv1->SetMinimum(0.88);
    ...
    cfv1 = (TH1D*)infile1->Get(Form("kstar_NumcqinvPAPtpcM0Psi3%s",kT));

    // cfv2 = (TH1D*)infile2->Get(Form("NumLongPckstarPAPtpcM0Psi%d%s",psibins2, kT));
    cfv2 = (TH1D*)infile2->Get(Form("NumLongPckstarPAPtpcM2Psi3%s",kT));

    cfv3 = (TH1D*)infile3->Get(Form("NumLongPckstarPAPtpcM4Psi3%s",kT));
    ...
    cfv1->SetMaximum(1.12);
    cfv1->SetMinimum(0.88);
    cfv1->GetXaxis()->SetRangeUser(0,0.25);
    ...
    cfv1 = (TH1D*)infile1->Get(Form("Numcqinv%stpcM0Psi3%s",system,kT));
    cfv2 = (TH1D*)infile2->Get(Form("Numcqinv%stpcM2Psi3%s",system,kT));
    cfv3 = (TH1D*)infile3->Get(Form("Numcqinv%stpcM4Psi3%s",system,kT));

    cout << Form("Numcqinv%stpcM0Psi%d%s",system,psibins2,kT) << endl;


    cfv1->SetMaximum(1.95);
    cfv1->SetMinimum(0.95);
    cfv1->GetXaxis()->SetRangeUser(0,0.25);
  ...
  cfv3->SetMarkerStyle(22);

  cfv1->SetName(Form("Numcqinv%stpcM0Psi3%s_1",system,kT));
  cfv2->SetName(Form("Numcqinv%stpcM2Psi3%s_2",system,kT));
  cfv3->SetName(Form("Numcqinv%stpcM4Psi3%s_3",system,kT));

  cfv1->SetMarkerColor(kRed);
  cfv2->SetMarkerColor(kBlack);
  cfv3->SetMarkerColor(kBlue);
...
TLegend *leg = new TLegend (0.3,0.7,0.85,0.85);
  ...
  else   if (strcmp(cmp,"cendep") == 0) {
    leg->AddEntry(cfv1,"0-10%","p");
    leg->AddEntry(cfv2,"10-30%","p");
    leg->AddEntry(cfv3,"30-50%","p");
  }
