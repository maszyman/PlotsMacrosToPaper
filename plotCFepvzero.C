...
void plotCFepvzero(const char* sys="PP", const char* kT="kT1", const char* proj="Out", int mult=2, TString dir = "../train_results_central_dcaptdep", int period=11, int numofpsibins = 6) {
...
// uncomment this to compare
  ...
  TFile* ofile = new TFile(Form("ofiles/cfmb%s%s%sc%d.root",sys,dir.Data(),kT,mult),"recreate");
  numpp[0][2][Nruns]->Write();
  // numsum->Write();
